...
local function LootLink_NameFromLink(link)
	...
	for name in string.gmatch(link, "|c%x+|Hitem:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+|h%[(.-)%]|h|r") do
		return name;
	end
	return nil;
end
...
--			item = string.gsub(itemLink.i, "^(%d+):(%d+):(%d+):(%d+)$", "%1:0:0:0:0:0:%3:%4:1");
			item = string.gsub(itemLink.i, "^(%d+):(%d+):(%d+):(%d+)$", "%1:0:0:0:0:0:%3:%4:1:0");
...
--			item = string.gsub(itemLink.i, "^(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(%d+)$", "%1:%2:%3:%4:%5:%6:%7:%8:1");
			item = string.gsub(itemLink.i, "^(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(%d+)$", "%1:%2:%3:%4:%5:%6:%7:%8:1:0");
		else
...
--			item = string.gsub(itemLink.i, "(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+)", "%1:0:0:0:0:%6:%7:%8:%9");
			item = string.gsub(itemLink.i, "(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+)", "%1:0:0:0:0:%6:%7:%8:%9:0"); -- 10th = reforge
		end
		return "item:"..item;
	end
	return nil;
...
function LootLink_OnLoad(self)
	...
	self:RegisterEvent("VARIABLES_LOADED");	-- canna be removed, now should use ADDON_LOADED arg1
...
-- hooksecurefunc("QuestFrameProgressItems_Update", LootLink_ScanQuest);	-- never fire
...
function LootLink_OnTooltipSetItem(self)
	local name, link = self:GetItem();
	LootLink_AddTooltipInfo(name, self);
end

function LootLink_OnTooltipSetItemNoPrice(self)
	local name, link = self:GetItem();
	LootLink_AddTooltipInfo(name, self, 0);
end
...
function LootLink_OnTooltipAddMoney(self, cost, maxcost)
	if( lBagID and lSlotID ) then
		LootLink_ScanSellPrice(cost);
	end
end
...
-- 335			if( ChatFrame1EditBox:IsVisible() ) then
-- 335				ChatFrame1EditBox:Insert(LootLink_GetLink(this:GetText()));
				local activeWindow = ChatEdit_GetActiveWindow();
				if ( activeWindow ) then
				activeWindow:Insert(LootLink_GetLink(self:GetText()));
			else
				ChatEdit_InsertLink(LootLink_GetLink(self:GetText()));
			end
		end
		if( IsControlKeyDown() ) then
			DressUpItemLink(LootLink_GetLink(self:GetText()));
		end
	elseif( button == "RightButton" ) then
		if( IsShiftKeyDown() and IsControlKeyDown() ) then
			--@todo Telo: add a confirmation dialog to this
			ItemLinks[self:GetText()] = nil;
			LootLink_Refresh();
		end
	end
end
...
function LootLinkItemButton_OnEnter(self)
	local link = LootLink_GetHyperlink(self:GetText());
	if( link ) then
		LootLinkFrame.TooltipButton = self:GetID();
		GameTooltip:SetOwner(self, "ANCHOR_BOTTOMRIGHT");
		local cached = LootLink_SetHyperlink(GameTooltip, self:GetText(), link);
...
function LootLinkFrameDropDownButton_OnClick(self)
	local oldID = UIDropDownMenu_GetSelectedID(LootLinkFrameDropDown);
	UIDropDownMenu_SetSelectedID(LootLinkFrameDropDown, self:GetID());
...
function LLS_RarityDropDown_OnClick(self)
	UIDropDownMenu_SetSelectedID(LLS_RarityDropDown, self:GetID());
end
...
function LLS_BindsDropDown_OnClick(self)
	UIDropDownMenu_SetSelectedID(LLS_BindsDropDown, self:GetID());
end
...
function LLS_LocationDropDown_OnClick(self)
	UIDropDownMenu_SetSelectedID(LLS_LocationDropDown, self:GetID());
end
...
function LLS_TypeDropDown_OnClick(self)
	local oldID = UIDropDownMenu_GetSelectedID(LLS_TypeDropDown);
	UIDropDownMenu_SetSelectedID(LLS_TypeDropDown, self:GetID());
	if( oldID ~= self:GetID() ) then
...
function LLS_SubtypeDropDown_OnClick(self)
	UIDropDownMenu_SetSelectedID(LLS_SubtypeDropDown, self:GetID());
end
...
-- called from: Atlasloot:AtlasLootItem_OnEnter, LootButtons.lua:86
end
-- should we do workaround or they will?
-- Best option is to disable it
function LootLink_AddItemOld(name, item, color)
...
function LLP(text)	--chatcopy
--print("LLP:"..text)
	local color, item, name
--	local color, item, name  string.gmatch(text, "|c(%x+)|Hitem:(%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+)|h%[(.-)%]|h|r")

--4.0.1: !cff1eff00!Hitem:12691:0:0:0:0:0:0:1260995456:52:0!h[Plans: Wildthorn Mail]!h!r
--4.0.1: |cff1eff00|Hitem:12691:0:0:0:0:0:0:1260995456:52:0|h[Plans: Wildthorn Mail]|h|r
--[19:29] !cff1eff00!Hitem:21841:0:0:0:0:0:0:1123309440:52:0!h[Netherweave Bag]!h!r
--[19:29] !cff1eff00!Hitem:21841:0:0:0:0:0:0:1123309440:52:0!h[Netherweave Bag]!h!r
	for color, item, name in string.gmatch(text, "|c(%x+)|Hitem:(%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+)|h%[(.-)%]|h|r") do
if color then
--print ("color:"..color)
end
if item then
--print ("item:"..item)
--print("link:"..string.gsub(item, "^(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+)$", "%1:0:0:0:0:%6:%7:%8:%9:0"))
--/print string.gsub("21841:0:0:0:0:0:0:1744761856:52:0", "^(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+)$", "%1:0:0:0:0:%6:%7:%8:%9:0")
end
if name then
--print ("name:"..name)
end
...
--print ("LootLink_ProcessLinks:"..text)	--chatcopy
--LLP(text)
	local color;
	local item;
	local name;
...
--4.0.1: !cff1eff00!Hitem:12691:0:0:0:0:0:0:1260995456:52:0!h[Plans: Wildthorn Mail]!h!r
OLNK="!cff1eff00!Hitem:12691:0:0:0:0:0:0:1260995456:52:0!h[Plans: Wildthorn Mail]!h!r"
--4.0.1: [Plans: Wildthorn Mail]
--/dump string.gmatch(OLNK, "!c(%x+)!Hitem:(%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+)|h%[(.-)%]!h!r")
--/print string.gmatch(OLNK, "!c(%x+)!Hitem:(%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+)|h%[(.-)%]!h!r")
--/script ZZZ,_,_=string.gmatch(OLNK, "!c(%x+)!Hitem:(%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+)|h%[(.-)%]!h!r")
--/print select(1, string.gmatch(OLNK, "!c(%x+)!Hitem:(%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+)|h%[(.-)%]!h!r"))



--real
--[22:28]  !cffffffff!Hitem:14047:0:0:0:0:0:0:1778109312:52:0!h[Runecloth]!h!r
--saved
--[22:36]  !cffffffff!Hitem:14047:0:0:0:0:0:0:1778109312:52:0:0!h[Runecloth]!h!r 5
--
--real		14047:0:0:0:0:0:0:1778109312:52:0
--[22:29]    i="14047:0:0:0:0:0:0:1778109312:52:0",
--              14047:0:0:0:0:0:0:1778109312:52:0


LASTSCANNED=string.gsub(text,"\|","!")
	for color, item, name in string.gmatch(text, "|c(%x+)|Hitem:(%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+)|h%[(.-)%]|h|r") do
			if( color and item and name and name ~= "" ) then
--print ("LootLink_ProcessLinks: has color,itemID,name:"..name)
				link = string.gsub(item, "^(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+)$", "%1:0:0:0:0:%6:%7:%8:%9:0");
				if( not ItemLinks[name] ) then
					ItemLinks[name] = { };
					ItemLinks[name].c = color;
		...
		for item, name in string.gmatch(text, "|Hitem:(%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+)|h%[(.-)%]|h") do
			if( item and name and name ~= "" ) then
				if( not ItemLinks[name] ) then
					ItemLinks[name] = { };
					ItemLinks[name].c = "ff40ffc0";
					ItemLinks[name].i = string.gsub(item, "^(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+)$", "%1:0:0:0:0:%6:%7:%8:%9:0");
					lItemLinksSizeTotal = lItemLinksSizeTotal + 1;
...
if lastName then
--print("return:"..lastName)	-- chatcopy
end
	return lastName;
end
