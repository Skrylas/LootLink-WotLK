--[[
	...
	LootLink 4.0: An in-game item database
		copyright 2004-2010 by Telo
...
lColorSortTable["ffe6cc80"] = 3;	-- heirloom, tan
lColorSortTable["ff0070dd"] = 4;	-- rare, blue
lColorSortTable["ff1eff00"] = 5;	-- uncommon, green
lColorSortTable["ffffffff"] = 6;	-- common, white
lColorSortTable["ff9d9d9d"] = 7;	-- poor, gray
...
local function LootLink_UpgradeLink(itemLink)
	if( string.find(itemLink.i, "^%-?%d+:%-?%d+:%-?%d+:%-?%d+$") ) then
		-- Upgrade pre 2.0 links
		itemLink.i = string.gsub(itemLink.i, "^(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+)$", "%1:0:0:0:0:0:%3:%4:0:0");
	elseif( string.find(itemLink.i, "^%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+$") ) then
		-- Upgrade pre 3.0 links
		itemLink.i = string.gsub(itemLink.i, "^(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+)$", "%1:0:0:0:0:0:%7:%8:0:0");
	elseif( string.find(itemLink.i, "^%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+$") ) then
		-- Upgrade pre 4.0 links
		itemLink.i = string.gsub(itemLink.i, "^(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+)$", "%1:0:0:0:0:0:%7:%8:0:0");
	end
end
...
local function LootLink_ScanQuest()
	...
	fQuestLog = QuestInfoFrame.questLog;
	if( fQuestLog ) then
...
function LootLink_AddItem(name, item, color)
	if( LL.lReady and color and item and name and name ~= "" ) then
		...
		if( string.find(item, "^%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+$") ) then
			-- Pre 4.0 link: remove instance-specific data from the given item (2nd is enchantment, 3rd-5th are sockets, 9th is the linking character's level
			itemString, valid = string.gsub(item, "^(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+)$", "%1:0:0:0:0:%6:%7:%8:0:0");
		else
			-- Remove instance-specific data from the given item (2nd is enchantment, 3rd-5th are sockets, 9th is the linking character's level, 10th is reforging)
			itemString, valid = string.gsub(item, "^(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+)$", "%1:0:0:0:0:%6:%7:%8:0:0");
		end
	...
	if( LL.lReady and text ) then
		for color, item, name in string.gmatch(text, "|c(%x+)|Hitem:(%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+:%-?%d+)|h%[(.-)%]|h|r") do
			if( color and item and name and name ~= "" ) then
				itemString, valid = string.gsub(item, "^(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+)$", "%1:0:0:0:0:0:%7:%8:0:0");
